...
const Skills = () => {
  ...
  const skillsLanguagesContent = () => {
    const SkillButton = (skill) => {
      const { name, img, exp, type, explain } = skill;
      const circumference = 2 * Math.PI * 45;
      const strokeDasharray = `${(circumference * exp) / 100} ${circumference}`;

      return (
        <div className="skill">
          <div className="skill-image" style={{ backgroundImage: `url(${img})` }}></div>
          <div className="skill-name">{name}</div>
          <svg className="skill-ring" viewBox="0 0 100 100">
            <circle className="ring-background" cx="50" cy="50" r="45"></circle>
            <circle
    ...
    return (
      <>
        <div className="skills_type_title">Languages</div>
        <div className="skills_type_table">
          <button>All</button>
          <button>Front</button>
          <button>Back</button>
        ...
        <div className="skills_type_contentBox">{SkillButton(skillsArray[0])}</div>
      </>
    );
  };
  ...
  const skillsPercentContent = () => {
    return (
      <ul>
        {(() => {
          const items = [];
          for (let i = 0; i < 10 && i < skillsArray.length; i++) {
            items.push(
              <li key={i}>
                <div className="about_info_rightBox_skills_name">{skillsArray[i].name}</div>
                <div
                  className={`about_info_rightBox_skills_percent percent_${skillsArray[i].exp}`}
                />
              </li>
            );
          }
