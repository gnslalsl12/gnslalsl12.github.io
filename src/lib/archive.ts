...
// `public/archive/<category>/<slug>.html`; a manifest at
// `public/archive/index.json` drives the hub listing.
//
...
export type ArchiveDoc = {
  ...
  description: string;
  path: string; // e.g. "archive/dev/vue-project-guide.html"
...
export async function fetchDocs(): Promise<ArchiveDoc[]> {
  const res = await fetch(`${BASE}archive/index.json?t=${Date.now()}`, { cache: "no-store" });
...
async function readSourceDocs(token: string, branch: string): Promise<ArchiveDoc[]> {
  const res = await fetch(`${REPO}/contents/public/archive/index.json?ref=${branch}`, {
...
// larger than the Contents API's ~1MB limit.
...
export async function publishDoc(input: PublishInput): Promise<ArchiveDoc> {
  ...
  const path = `archive/${input.category}/${input.slug}.html`;
  ...
  await commitFiles(
    ...
    [
      ...
      { path: `public/archive/index.json`, content: manifest },
    ],
